/* ============================================================
   ...
   Variant B — Guided / Card-Based Triage
...
const DATA = (function () {
  ...
  // ---- Tickets (12) -------------------------------------------------------
  // turnaround: days from created to resolved (null if open)
  function tl(steps) { return steps; } // status timeline helper
  ...
  // Each links to a ticket (one ticket may cover several exceptions).
  function ex(o) {
  ...
  const exceptions = [
    ex({ id: "EX-0001", type: "NULL_PO_NUMBER", facility: "DIS-ATL-01", system: "Ship Hero",
        ticket: "WIQ-1041", status: "In Progress",
        headline: "This receipt at DIS-ATL-01 is missing its PO number",
    ...
    ex({ id: "EX-0007", type: "THREE_WAY_MATCH_VARIANCE", facility: "CK-CHI-01", system: "Fishbowl",
        ticket: "WIQ-1044", status: "In Progress",
    ...
    ex({ id: "EX-0009", type: "TRANSFER_WAREHOUSE_IMBALANCE", facility: "TW-001", system: "Ship Hero",
        ticket: "WIQ-1045", status: "In Review",
    ...
    ex({ id: "EX-0012", type: "NEGATIVE_ON_HAND", facility: "IK-NYC-01", system: "Pantry",
        ticket: "WIQ-1046", status: "Resolved",
  ...
  // ---- Dashboard KPIs (consistent with exceptions/tickets) ----------------
  const kpis = {
    openExceptions: 22,            // total exceptions in this run
    newToday: 22,                  // all from the 2026-06-07 run
    autoClosedToday: 3,            // WIQ-1031, WIQ-1027, WIQ-1002 within recent window
    avgTurnaroundDays: 2.7,        // across resolved/closed tickets
    pctWithinSla: 78               // % resolved within SLA target
  };
  ...
  // avgDays, within, breaching computed against closed/resolved tickets;
  // numbers tuned so Procurement clearly needs attention.
  const slaByTeam = [
  ...
  // aging buckets across currently-open tickets
  const aging = [
  ...
  // overdue tickets (past SLA target, still open)
  const overdue = ["WIQ-1043", "WIQ-1044", "WIQ-1019"];

  // ---- Derived: errors by type & facility (from exceptions) ---------------
  function countBy(arr, key) {
    const m = {};
    arr.forEach(e => { m[e[key]] = (m[e[key]] || 0) + 1; });
    return Object.keys(m).map(k => ({ key: k, count: m[k] })).sort((a, b) => b.count - a.count);
  }
  const errorsByType = countBy(exceptions, "type");
  const errorsByFacility = countBy(exceptions, "facility");

  return {
    runDate: "2026-06-07", today: "2026-06-08",
    teams, facilities, systems, sla, rules, rulesById,
    tickets, ticketsByKey, exceptions, recurrence,
