...
- **In-app actions push to Jira (bidirectional-ish):** from the exception drawer you can change **status** (Open → In Progress → Done; transitions the real issue), **reassign** (sets the Jira assignee), **sub-assign** (Jira comment), and **mark resolved** (Jira → Done). Status filter is data-driven.
